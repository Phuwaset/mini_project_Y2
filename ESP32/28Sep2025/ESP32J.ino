...
void runArmSequence_B(){
  moveServoSmooth(servoRboArm1, currentArm1, 5, 15);
  moveServoSmooth(servoRboArm2, currentArm2, 155, 15);
...
//=============== Point robot arm C ==========
void runArmSequence_C(){
  moveServoSmooth(servoRboArm1, currentArm1, 50, 15);
  moveServoSmooth(servoRboArm2, currentArm2, 155, 15);
  moveServoSmooth(servoRboArm3, currentArm3, 50, 15);
  moveServoSmooth(servoRboArm4, currentArm4, 150, 15);
  Serial.println("Pose 1: (30,155,65,150)");
  delay(1000);
}
...
//=============== Point robot arm D ==========
void runArmSequence_D(){
  moveServoSmooth(servoRboArm1, currentArm1, 5, 15);
  moveServoSmooth(servoRboArm2, currentArm2, 130, 15);
  moveServoSmooth(servoRboArm3, currentArm3, 50, 15);
  moveServoSmooth(servoRboArm4, currentArm4, 150, 15);
  Serial.println("Pose 1: (30,155,65,150)");
  delay(1000);
...
void loop() {
  ...
  // ❗ ไม่ต้องให้จบที่ IDLE ทุกครั้ง → ปล่อย RUNNING ไว้ แล้วค่อย RESET เองด้วยปุ่ม
  if (sysState == RUNNING) {
    // ----------- เช็ก Lead Flags -----------
    if (L_PointA) {
      Serial.println("[LEAD] L_A=ON");
      pose2_Robot_lead();
      safeDelay(500);
      ...
      safeDelay(500);
      L_PointA = false;
      Serial.println("✅ L_A sequence done");
    }

    if (L_PointB) {
      Serial.println("[LEAD] L_B=ON");
      pose3_Robot_lead();
      L_PointB = false;
      Serial.println("✅ L_B sequence done");
    }

    if (L_PointC) {
      Serial.println("[LEAD] L_C=ON");
      pose4_Robot_lead();
      safeDelay(500);
      pose2_Robot_lead();
      safeDelay(500);
      ...
      L_PointC = false;
      Serial.println("✅ L_C sequence done");
    }

    // ----------- เช็ก Arm Flags -----------
    if (A_Point1) {
      Serial.println("[ARM] A_1=ON");
      runArmSequence();
      A_Point1 = false;
      Serial.println("✅ A_1 sequence done");
    }

    if (A_PointA) {
      Serial.println("[ARM] A_A=ON");
      runArmSequence_A();
      safeDelay(500);
      pose_home_robotArm();
      pose_home_lead_screw(); 
      A_PointA = false;
      Serial.println("✅ A_A sequence done");
    }

    if (A_PointB) {
      Serial.println("[ARM] A_B=ON");
      runArmSequence_B();
      safeDelay(500);
      pose_home_robotArm();
      pose_home_lead_screw(); 
      A_PointB = false;
      Serial.println("✅ A_B sequence done");
    }

    if (A_PointC) {
      Serial.println("[ARM] A_C=ON");
      runArmSequence_C();
      safeDelay(500);
      pose_home_robotArm();
      pose_home_lead_screw(); 
      A_PointC = false;
      Serial.println("✅ A_C sequence done");
    }

    if (A_PointD) {
      Serial.println("[ARM] A_D=ON");
      runArmSequence_D();
      safeDelay(500);
      pose_home_robotArm();
      pose_home_lead_screw(); 
      A_PointD = false;
      Serial.println("✅ A_D sequence done");
    }
  }
}
...
// ============================================================
// Read Serial from MEGA (Event-driven)
// ============================================================
void readSerialFromMega() {
  while (U2.available()) {
    ...
    if (c == '\n') {
      ...
      if (strncmp(inputBuffer, "DIST:", 5) == 0) {
        ...
        Serial.println("[CMD] Emergency received!");
        R_EMG = true;
      }
      ...
      else if (strncmp(inputBuffer, "A_", 2) == 0) {
        // 👉 Robot Arm event
        Serial.print("[ARM] ");
        Serial.println(inputBuffer);   // เช่น "A_H=ON" หรือ "A_B=OFF"
        parseRelayEvent(inputBuffer, true); // true = Arm
      }
      else if (strncmp(inputBuffer, "L_", 2) == 0) {
        // 👉 Robot Lead event
        Serial.print("[LEAD] ");
        Serial.println(inputBuffer);   // เช่น "L_H=ON" หรือ "L_B=OFF"
        parseRelayEvent(inputBuffer, false); // false = Lead
      }
      else {
 ...
 // ---------------- ฟังก์ชันแยกข้อความ ----------------
void parseRelayEvent(const char* msg, bool isArm) {
  // msg เช่น "A_H=ON" หรือ "L_B=OFF"

  String s = String(msg);
  int eqPos = s.indexOf('=');
  if (eqPos == -1) return;

  String label = s.substring(0, eqPos);   // เช่น "A_H"
  String value = s.substring(eqPos + 1);  // "ON" หรือ "OFF"
  bool state = (value == "ON");

  if (isArm) {
    if (label == "A_H")   A_Home = state;
    else if (label == "A_1") A_Point1 = state;
    else if (label == "A_A") A_PointA = state;
    else if (label == "A_B") A_PointB = state;
    else if (label == "A_C") A_PointC = state;
    else if (label == "A_D") A_PointD = state;
  } else {
    if (label == "L_H")   L_Home = state;
    else if (label == "L_A") L_PointA = state;
    else if (label == "L_B") L_PointB = state;
    else if (label == "L_C") L_PointC = state;
  }
}
...
void safeDelay(unsigned long ms) {
  ...
  while (millis() - start < ms) {
    checkButtons();    // ให้ Emergency ยังทำงาน
    client.loop();     // ให้ MQTT ยังทำงาน
  }
}
// ============================================================
